...
/**
 * test-all.mjs — Comprehensive test suite for freelance-ops
 *
 ...
 * any freshly cloned freelance-ops checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the freelance-ops repository root.
...
const scripts = [
  ...
  // their active freelance-ops workspace.
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'freelance-ops-portals-validator-'));
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'freelance-ops-migrate-'));
  try {
    ...
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, freelance_ops_TRACKER: tracker } });
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'freelance-ops-dedup-'));
  try {
    ...
    const dedupResult = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, freelance_ops_TRACKER: tracker } });
    if (dedupResult === null) {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'freelance-ops-merge-'));
  try {
    ...
    const mergeResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, freelance_ops_TRACKER: tracker, freelance_ops_ADDITIONS: additionsDir } });
    if (mergeResult === null) {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'freelance-ops-merge-lock-'));
  /**
  ...
  function spawnMerge(additionsDir, holdMs = 0) {
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...process.env,
          freelance_ops_TRACKER: join(mergeTmp, 'data', 'applications.md'),
          freelance_ops_ADDITIONS: additionsDir,
          freelance_ops_TRACKER_LOCK: join(mergeTmp, 'freelance-ops-merge-tracker-fixture.lock'),
          freelance_ops_MERGE_HOLD_MS: String(holdMs),
          freelance_ops_MERGE_READY_IPC: '1',
        },
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'freelance-ops-index-'));
    try {
      const md = join(idxTmp, 'applications.md');
      const env = { ...process.env, freelance_ops_TRACKER: md };
...
try {
  ...
  const hit = sj.detect({ name: 'SJ', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=freelance-ops' });
  ...
  const fakeJobs = {
    jobs: [
      { title: 'Senior Dev', url: 'https://solid.jobs/o/abc123/freelance-ops', company: 'Acme Corp', locations: ['Warszawa', 'Remote'] },
      { title: 'Junior Dev', url: 'https://solid.jobs/o/def456/freelance-ops', company: 'Beta Inc', locations: ['Kraków'] },
    ],
  };
  const parsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=freelance-ops' },
  ...
  if (parsed[0].title === 'Senior Dev' && parsed[0].url === 'https://solid.jobs/o/abc123/freelance-ops') {
  ...
  const noCompanyJobs = { jobs: [{ title: 'Tester', url: 'https://solid.jobs/o/xyz/freelance-ops', locations: [] }] };
  ...
  const mixedJobs = {
    jobs: [
      { title: 'Has URL', url: 'https://solid.jobs/o/1/freelance-ops', company: 'A', locations: [] },
  ...
  const stringLocJobs = { jobs: [{ title: 'Dev', url: 'https://solid.jobs/o/2/freelance-ops', company: 'X', locations: 'Warsaw' }] };
  ...
  const malformedMembers = { jobs: [null, 7, { title: 'OK', url: 'https://solid.jobs/o/3/freelance-ops', company: 'Z' }] };
  const safeParsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=freelance-ops' },
    { transport: 'http', fetchJson: async () => malformedMembers, fetchText: async () => '' },
  );
  if (safeParsed.length === 1 && safeParsed[0].url === 'https://solid.jobs/o/3/freelance-ops') {
...
try {
  ...
  // Release Please tags carry the component prefix (freelance-ops-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('freelance-ops-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (freelance-ops-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on freelance-ops-v1.9.0 (got ${parse('freelance-ops-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('freelance-ops') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (freelance-ops → ${parse('freelance-ops')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
