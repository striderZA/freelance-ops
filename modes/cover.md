...
Works in two modes:
- **Slug mode:** `/freelance-ops cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/freelance-ops cover` or JD pasted directly — starts from scratch
...
When invoked as `/freelance-ops cover {slug}`:
