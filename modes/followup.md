...
> "No active applications to follow up on. Apply to some roles first with `/freelance-ops` and come back when they're aging."
...
- Suggest the user run `/freelance-ops contacto {company}` to find the right person first
...
> - Trying a different contact via `/freelance-ops contacto`
...
**To:** {email or "No contact found — run `/freelance-ops contacto` first"}
