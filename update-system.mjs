...
/**
 * update-system.mjs — Safe auto-updater for freelance-ops
 *
...
const SYSTEM_PATHS = [
  ...
  // Multi-language mode directories
  'modes/ar/',
  'modes/de/',
  'modes/fr/',
  'modes/ja/',
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: freelance-ops-update-checker',
    ]),
  ]);
...
async function apply() {
  ...
  try {
    ...
    // See: https://github.com/santifer/freelance-ops/issues/649
