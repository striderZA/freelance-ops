...
// relative to freelanceopsPath. Links are normally relative to the tracker
...
func resolveReportPath(freelanceopsPath, trackerPath, link string) string {
	resolved := filepath.Join(filepath.Dir(trackerPath), link)
	if _, err := os.Stat(resolved); err != nil {
		legacy := filepath.Join(freelanceopsPath, link)
		if _, err2 := os.Stat(legacy); err2 == nil {
			resolved = legacy
		}
	}
	if rel, err := filepath.Rel(freelanceopsPath, resolved); err == nil {
		return rel
	}
	return link
}
...
func ParseApplications(freelanceopsPath string) []model.CareerApplication {
	...
	batchURLs := loadBatchInputURLs(freelanceopsPath)
	reportNumURLs := loadJobURLs(freelanceopsPath)

	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		fullReport := filepath.Join(freelanceopsPath, apps[i].ReportPath)
	...
	enrichFromScanHistory(freelanceopsPath, apps)

	// Strategy 5: company name fallback from batch-input.tsv
	enrichAppURLsByCompany(freelanceopsPath, apps)

	return apps
}

// loadBatchInputURLs reads batch-input.tsv and returns a map of batch ID -> job URL.
func loadBatchInputURLs(freelanceopsPath string) map[string]string {
	inputPath := filepath.Join(freelanceopsPath, "batch", "batch-input.tsv")
...
func loadJobURLs(freelanceopsPath string) map[string]string {
	...
	statePath := filepath.Join(freelanceopsPath, "batch", "batch-state.tsv")
...
func enrichFromScanHistory(freelanceopsPath string, apps []model.CareerApplication) {
	scanPath := filepath.Join(freelanceopsPath, "scan-history.tsv")
...
func enrichAppURLsByCompany(freelanceopsPath string, apps []model.CareerApplication) {
	inputPath := filepath.Join(freelanceopsPath, "batch", "batch-input.tsv")
	inputData, err := os.ReadFile(inputPath)
	if err != nil {
		return
...
func LoadReportSummary(freelanceopsPath, reportPath string) (archetype, tldr, remote, comp string) {
	fullPath := filepath.Join(freelanceopsPath, reportPath)
