#!/usr/bin/env node
// freelance-ops scaffolder — one-command install.
...
const REPO = "https://github.com/santifer/freelance-ops.git";
const LATEST_RELEASE = "https://api.github.com/repos/santifer/freelance-ops/releases/latest";
const NPM = process.platform === "win32" ? "npm.cmd" : "npm";

// freelance-ops is AI-agnostic: every one of these CLIs reads AGENTS.md and works
...
const USAGE = `freelance-ops — set up an AI job search workspace.

Usage:
  npx freelance-ops init [folder]    Create a new workspace (default: ./freelance-ops)

After setup, open your AI coding tool inside the folder and paste a job offer.
Docs: https://github.com/santifer/freelance-ops`;
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "freelance-ops-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  const target = dirArg || "freelance-ops";
  ...
  // Pretty path for messages: "./freelance-ops" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning freelance-ops${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  console.log(`\n✓ freelance-ops is ready in ${display}\n`);
  ...
  console.log("\nfreelance-ops is AI-agnostic — Claude Code, Gemini, Codex, Qwen, OpenCode and Copilot all work.");
