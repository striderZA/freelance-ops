...
import (
	...
	"github.com/santifer/freelance-ops/dashboard/internal/data"
...
func NewPipelineModel(t theme.Theme, apps []model.CareerApplication, metrics model.PipelineMetrics, freelanceopsPath string, width, height int) PipelineModel {
	m := PipelineModel{
		apps:          apps,
		metrics:       metrics,
		sortMode:      sortScore,
...
func (m PipelineModel) WithReloadedData(apps []model.CareerApplication, metrics model.PipelineMetrics) PipelineModel {
	...
	reloaded := NewPipelineModel(m.theme, apps, metrics, m.freelanceopsPath, m.width, m.height)
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "enter":
		if app, ok := m.CurrentApp(); ok && app.ReportPath != "" {
			fullPath := filepath.Join(m.freelanceopsPath, app.ReportPath)
		...
		if app, ok := m.CurrentApp(); ok {
			...
			return m, func() tea.Msg {
				return PipelineUpdateStatusMsg{
					freelanceopsPath: m.freelanceopsPath,
					App:           app,
					NewStatus:     newStatus,
				}
			}
...
func (m PipelineModel) loadCurrentReport() tea.Cmd {
	...
	path := m.freelanceopsPath
	report := app.ReportPath
	return func() tea.Msg {
		return PipelineLoadReportMsg{freelanceopsPath: path, ReportPath: report}
	}
}
...
func (m PipelineModel) renderHelp() string {
	...
	brand := lipgloss.NewStyle().Foreground(m.theme.Overlay).Render("freelance-ops by santifer.io")
