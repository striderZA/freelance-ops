...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/freelance-ops cover {slug}`.
...
> Draft generated at evaluation time. Complete via `/freelance-ops cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/freelance-ops cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
