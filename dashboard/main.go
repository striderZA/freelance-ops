...
import (
	...
	"github.com/santifer/freelance-ops/dashboard/internal/ui/screens"
)
...
func (m *appModel) reloadPipelineData() {
	apps := data.ParseApplications(m.freelanceopsPath)
...
func (m appModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case screens.PipelineLoadReportMsg:
		archetype, tldr, remote, comp := data.LoadReportSummary(msg.freelanceopsPath, msg.ReportPath)
		m.pipeline.EnrichReport(msg.ReportPath, archetype, tldr, remote, comp)
		return m, nil

	case screens.PipelineUpdateStatusMsg:
		err := data.UpdateApplicationStatus(msg.freelanceopsPath, msg.App, msg.NewStatus)
		if err != nil {
...
func main() {
	pathFlag := flag.String("path", ".", "Path to freelance-ops directory")
	flag.Parse()

	freelanceopsPath := *pathFlag
	...
	pm := screens.NewPipelineModel(t, apps, metrics, freelanceopsPath, 120, 40)

	for _, app := range apps {
		if app.ReportPath == "" {
			continue
		}
		archetype, tldr, remote, comp := data.LoadReportSummary(freelanceopsPath, app.ReportPath)
