#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for freelance-ops
 *
...
const APPS_FILE = existsSync(join(freelance_ops, 'data/applications.md'))
  ? join(freelance_ops, 'data/applications.md')
  : join(freelance_ops, 'applications.md');
const FOLLOWUPS_FILE = join(freelance_ops, 'data/follow-ups.md');
const PROFILE_FILE = process.env.freelance_ops_PROFILE || join(freelance_ops, 'config/profile.yml');
...
function resolveReportPath(reportField) {
  ...
  const fullPath = join(freelance_ops, match[1]);
