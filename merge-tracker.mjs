 ...
 *
 * Run: node freelance-ops/merge-tracker.mjs [--dry-run] [--verify]
 */
...
// freelance_ops_ADDITIONS overrides the additions dir (used by tests, mirrors freelance_ops_TRACKER).
...
const MERGE_HOLD_MS = Number(process.env.freelance_ops_MERGE_HOLD_MS) || 0;
const MERGE_READY_IPC = process.env.freelance_ops_MERGE_READY_IPC === '1';

const trackerLockKey = createHash('sha256').update(APPS_FILE).digest('hex').slice(0, 16);
const TRACKER_LOCK_DIR = resolveTrackerLockDir(process.env.freelance_ops_TRACKER_LOCK, trackerLockKey);
 ...
 *
 * `freelance_ops_TRACKER_LOCK` exists for tests and unusual local layouts, but the
 ...
 * and use the freelance-ops lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `freelance-ops-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('freelance-ops-merge-tracker-')) return fallback;
  return candidate;
}
 ...
 * - the regression test hook (`freelance_ops_MERGE_HOLD_MS`), which deliberately
...
try {
  trackerLock = await acquireTrackerLock(TRACKER_LOCK_DIR, {
    timeoutMs: Number(process.env.freelance_ops_TRACKER_LOCK_TIMEOUT_MS) || 60_000,
    retryMs: Number(process.env.freelance_ops_TRACKER_LOCK_RETRY_MS) || 75,
    staleMs: Number(process.env.freelance_ops_TRACKER_LOCK_STALE_MS) || 10 * 60_000,
  });
...
if (MIGRATE) {
  ...
  if (DRY_RUN) {
    ...
    console.log(`✅ Migration: rewrote ${changed} report link(s) in ${basename(APPS_FILE)} relative to ${TRACKER_DIR === freelance_ops ? 'repo root' : 'data/'}`);
  }
  process.exit(0);
}
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(freelance_ops, 'verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}
